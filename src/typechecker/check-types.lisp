...
(defmethod check-node-type ((node typed-node-literal) env)
  (with-accessors ((scheme typed-node-type)
                   (value typed-node-literal-value))
      node
    (if (type-scheme= scheme *lisp-object-scheme*)
        *lisp-object-scheme*
        (multiple-value-bind (type preds)
            (derive-literal-type value)
          (let ((derived-scheme (to-scheme (qualify preds type))))
            (unless (type-scheme= derived-scheme scheme)
              (error 'invalid-typed-node-type
                     :node node
                     :inferred-type type))
            derived-scheme)))))

(defmethod check-node-type ((node typed-node-variable) env)
...
(defmethod check-node-type ((node typed-node-lisp) env)
  (typed-node-type node))

(defmethod check-node-type ((node typed-node-application) env)
  (check-application-node-type
   (typed-node-application-rator node)
   (typed-node-application-rands node)
   node env))

(defmethod check-node-type ((node typed-node-direct-application) env)
  ;; TODO: something else here
  (dolist (rand (typed-node-direct-application-rands node))
    (check-node-type rand env))
  (typed-node-type node))

(defmethod check-node-type ((node typed-node-abstraction) env)
...
(defmethod check-node-type ((node typed-node-let) env)
...
(defmethod check-node-type ((node typed-node-match) env)
...
(defmethod check-node-type ((node typed-node-seq) env)
